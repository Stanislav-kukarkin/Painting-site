...
"use strict";
__webpack_require__.r(__webpack_exports__);
var calc = function calc(size, mat, opc, promo, result) {
...
var forms = function forms() {
  ...
  upload.forEach(function (item) {
    item.addEventListener("input", function () {
      ...
      arr[0].length > 5 ? dots = "..." : dots = "."; //split разбивает строку "abcd.ru" => ["abcd", "ru"]
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var api;
      item.closest('.popup-design') || item.classList.contains("calc_form") ? api = path.designer : api = path.question; //closest ищет выше по иерархии
    ...
    link.addEventListener('click', function (event) {
      ...
      function step(time) {
        ...
        var progress = time - start,
            r = toBlock < 0 ? Math.max(widthTop - progress / speed, widthTop + toBlock) : Math.min(widthTop + progress / speed, widthTop + toBlock); //количество пикселей необходимых что бы пролистать за анимацию

        document.documentElement.scrollTo(0, r); // 0-ось х r-ось y
